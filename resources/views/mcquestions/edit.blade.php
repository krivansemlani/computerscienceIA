    ...
    <div class="container">
        ...
        <form action="{{ route('mcquestions.update', $mcquestion->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group" class="scrolling-container"
                style="max-height: 500px; overflow-y: auto; border: 3px solid #000;">
                <input type="file" class="form-control" id="QImage" name="QImage" accept="image/*"
                    onchange="previewImage()" style="padding: 10px 0;">
                <img id="image-preview" src="{{ asset('storage/' . $mcquestion->QImage) }}" alt="Question Image"
                    style="max-width: 100%; height: auto;">
            </div>

            <div class="form-group">
                <label for="Option1">Option 1</label>
                <input type="text" class="form-control" id="Option1" name="Option1" value="{{ $mcquestion->Option1 }}"
                    required>
            </div>
            <div class="form-group">
                <label for="Option2">Option 2</label>
                <input type="text" class="form-control" id="Option2" name="Option2" value="{{ $mcquestion->Option2 }}"
                    required>
            </div>
            <div class="form-group">
                <label for="Option3">Option 3</label>
                <input type="text" class="form-control" id="Option3" name="Option3" value="{{ $mcquestion->Option3 }}"
                    required>
            </div>
            <div class="form-group">
                <label for="Option4">Option 4</label>
                <input type="text" class="form-control" id="Option4" name="Option4" value="{{ $mcquestion->Option4 }}"
                    required>
            </div>
                ...
                <select class="form-control" id="subject_id" name="subject_id" required>
                    @foreach ($subjects as $subject)
                        <option value="{{ $subject->id }}"
                            {{ $subject->id == $mcquestion->chapter->subject->id ? 'selected' : '' }}>
                            {{ $subject->SName }}
                        </option>
                    @endforeach
                </select>
                ...
                <select class="form-control" id="chapter_id" name="chapter_id" required>
                    @foreach ($chapters as $chapter)
                        <option value="{{ $chapter->id }}"
                            {{ $chapter->id == $mcquestion->chapter->id ? 'selected' : '' }}>
                            {{ $chapter->CName }}
                        </option>
                    @endforeach
                </select>
